...
class DtiGlobalFunctionStockReport(models.TransientModel):
    ...
    def _get_stock_card(self, company_id, start_date, end_date, location_id, products, warehouse_id=False):
        DATA_REPORT = []
        ...
        if products:
            ...
            product_ids = self.env['product.product'].with_context(location=location_id,to_date=tmp_start_date,warehouse_id=warehouse_id).browse(products)
        else:
            
            product_ids = self.env['product.product'].with_context(location=location_id, to_date=tmp_start_date,warehouse_id=warehouse_id).search([
                '|',('company_id', '=', False),
        ...
        if warehouse_id:
            ...
            domain_valuation_layers += ['|',
                                        ('location_id.warehouse_id', '=', warehouse_obj.id),
                                        ('location_dest_id.warehouse_id', '=', warehouse_obj.id)
                                       ]
            ...
            for move in move_ids:
                ...
                if _is_out(move):
                    ...
                    if location_obj:
                        ...
                        if move.location_id.warehouse_id == warehouse_obj and move.location_dest_id.warehouse_id != warehouse_obj:
                            quantity = -quantity
                        elif move.location_id.warehouse_id != warehouse_obj and move.location_dest_id.warehouse_id == warehouse_obj:
                            quantity = quantity
                        else:
                            continue
                    else:
                        # No filter (All view): show IT as Transfer Out + Transfer In (net zero on balance)
                        is_mutation = True
                        partner_name = move.partner_id.display_name or (move.picking_id.partner_id.display_name if move.picking_id else '')
                        DATA_REPORT_LINE.append({
                            'seq'           : seq,
                            ...
                            'balance_qty'   : balance_qty - move.quantity,
                ...
                DATA_REPORT_LINE.append({
                    ...
                    'balance_qty'       : balance_qty,
                    'partner_id'        : move.partner_id.display_name or move.picking_id.partner_id.display_name,
                })
